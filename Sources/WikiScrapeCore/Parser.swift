...
public struct Parser {
    //keywords that are in the wikipedia html organization.
    //If this stops working, check their site to see the path to the content is the same
    ...
    func parseData(_ html: String, _ rowsRequested: Int) throws -> [Track]{
        ...
        let numberOfUseableRows = try useableRows(tbody)
    ...
    func write(inputFile: String, outputFile: URL) throws{
        guard outputFile.path.isEmpty == false else {throw WriteError.emptyPath}
        guard inputFile.isEmpty == false else {throw WriteError.emptyInput}
        guard outputFile.pathExtension.count > 0 else {throw WriteError.noExt}
        
        //checks if the path -file is valid
        do{
            var pathBase = outputFile
        ...
        do{
            if ((try? outputFile.checkResourceIsReachable() == true) != nil) {
                print("Writing over \(outputFile.lastPathComponent)")
            }
    ...
    public func parse(_ url: String, _ rows: Int, _ outputFile: URL) throws {
